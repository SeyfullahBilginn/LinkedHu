...
function EventDetail() {
  ...
  function addOrRemoveLike() {
    LikeService.getEventLikes(userCookie.user.id, event.id)
      .then(() => {
        if (isLiked) {
  ...
  function click() {
    CommentService.addEventComment(userCookie.user.id, event.id, comment)
      .then(() => {
        setCommentsNum(comment + 1);
      })
      .catch((err) => {
        console.log(err);
      });
          ...
          <SuiBox pr={1}>
            <SuiBox display="flex" flexDirection="row" alignItems="center">
              <SuiInput
                ...
                text="asd"
                // icon={{ component: "comment", direction: "left" }}
                onChange={(e) => {
